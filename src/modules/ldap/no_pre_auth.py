from rich.console import Console
from handlers.ldap_connection import LdapHandler

console = Console()


class NoPreAuth:
    name = "no_pre_auth"
    desc = "Find all users that do not require Kerberos pre-authentication"
    module_protocol = ["ldap"]
    opsec_safe = True
    multiple_hosts = False
    user_target = None
    search_filter = "(&(objectclass=user)(objectcategory=user)(useraccountcontrol:1.2.840.113556.1.4.803:=4194304))"
    requires_args = False
    attributes = ['sAMAccountName', 'distinguishedName']

    def on_login(self):
        conn, base_dn = LdapHandler.connection(self)
        results = conn.search(base_dn, self.search_filter, attributes=self.attributes)
        res_status = results[0]
        res_response = results[2]

        if res_status:
            console.print("[green][+][/] No Pre Auth Required:")
            
            for entry in res_response:
                if entry["type"] == "searchResEntry":
                    username = entry["attributes"]["sAMAccountName"]
                    object_dn = entry["attributes"]["distinguishedName"]
                    console.print(f"[cyan]-[/] {username} ({object_dn})", highlight=False)
        else:
            console.print("[red][!][/] No entries found in the results.")
